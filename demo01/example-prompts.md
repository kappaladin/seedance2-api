# Seedance 2.0 分镜提示词示例库

> **引用语法说明：** 新版 API 使用 `@image_file_N`、`@video_file_N`、`@audio_file_N` 引用素材（也兼容旧版 `@图片N`、`@视频N`、`@音频N`）。`image_files` 数组中第 N 个元素对应 `@image_file_N`，`video_files` 和 `audio_files` 同理。

## 示例一：情感叙事类

**场景**：男人下班回家的温情故事

```
电影级写实风格，15秒，2.35:1电影宽屏，温馨家庭氛围

0-3秒：中景跟随镜头，男人疲惫地走在走廊，脚步逐渐变缓，最后停在家门口
3-5秒：脸部特写，男人深呼吸，调整情绪，收起负面情绪，表情变得轻松
5-7秒：特写手部动作，翻找出钥匙，插入门锁，门打开
7-12秒：室内中景，小女儿和宠物狗欢快地跑过来迎接，男人蹲下拥抱他们
12-15秒：近景，男人脸上洋溢着幸福的笑容，室内暖光营造温馨氛围

【声音】轻缓的钢琴配乐，环境音效（脚步声、开门声、孩子的笑声）
【参考】@image_file_1 男人形象，@image_file_2 女儿形象，@image_file_3 宠物狗形象
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "image_files": ["男人形象URL", "女儿形象URL", "宠物狗形象URL"],
  "ratio": "21:9",
  "duration": 15
}
```

---

## 示例二：动作打斗类

**场景**：武侠风格双人对打

```
中国水墨武侠风格，15秒，16:9，枫叶飘落的秋季场景

0-2秒：远景，两位侠客对峙，一人持长枪（参考@image_file_1 @image_file_2），一人持双刀（参考@image_file_3 @image_file_4）
2-4秒：快速推近，两人眼神交锋，杀气弥漫
4-9秒：中景快速剪辑，长枪突刺，双刀格挡，武器碰撞火花四溅，模仿@video_file_1的动作节奏
9-12秒：环绕镜头，展示激烈打斗，枫叶被气浪卷起飞舞
12-15秒：定格pose，两人武器相交，画面渐隐

【声音】金属碰撞音效 + 古风激昂配乐
【参考】@image_file_1-4 角色造型，@image_file_5 枫叶林场景，@video_file_1 动作参考
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "image_files": ["角色造型1URL", "角色造型2URL", "角色造型3URL", "角色造型4URL", "枫叶林场景URL"],
  "video_files": ["动作参考视频URL"],
  "ratio": "16:9",
  "duration": 15
}
```

---

## 示例三：产品广告类

**场景**：咖啡品牌广告

```
高端商业广告风格，15秒，16:9，暖色调晨光氛围

0-3秒：微距特写，咖啡液缓缓注入杯中，油脂丰富，蒸汽升腾
3-6秒：中景环绕，手握咖啡杯，阳光透过窗户洒在桌面
6-10秒：推镜头至咖啡豆，一粒咖啡豆从上方飘落，镜头跟随推进
10-12秒：画面黑屏转场
12-15秒：文字渐显，第一行"Lucky Coffee"，第二行"Breakfast"，第三行"AM 7:00-10:00"

【声音】咖啡倒入声 + 轻松的爵士乐
【参考】@image_file_1 咖啡杯，@image_file_2 品牌logo
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "image_files": ["咖啡杯URL", "品牌logoURL"],
  "ratio": "16:9",
  "duration": 15
}
```

---

## 示例四：创意转场类

**场景**：科幻世界穿梭

```
赛博朋克科幻风格，15秒，16:9，霓虹光效

0-2秒：首帧@image_file_1，人物戴上虚拟科幻眼镜，特写
2-5秒：极速环绕镜头，从第三人称变成人物主观视角，在AI虚拟眼镜中穿梭
5-8秒：镜头推进至深邃蓝色宇宙（参考@image_file_2），几架飞船穿梭向远方，镜头跟随
8-11秒：飞船急速穿梭到像素世界（参考@image_file_3），低空飞过像素山林，树木生长
11-13秒：视角仰拍，急速穿梭到浅绿色纹理星球（参考@image_file_4）
13-15秒：镜头掠过星球表面，画面定格

【声音】电子音效 + 科幻氛围配乐
【参考】@image_file_1 首帧人物，@image_file_2-4 场景图，@video_file_1 运镜参考
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "image_files": ["首帧人物URL", "宇宙场景URL", "像素世界URL", "星球URL"],
  "video_files": ["运镜参考视频URL"],
  "ratio": "16:9",
  "duration": 15
}
```

---

## 示例五：音乐卡点类

**场景**：时尚换装展示

```
时尚MV风格，15秒，9:16竖屏，高饱和炫彩

0-2秒：快速四格闪切，红、粉、紫、豹纹四款造型依次定格
2-4秒：音乐重拍，第一款造型全身展示，鱼眼镜头效果
4-6秒：音乐重拍，快速切换第二款造型，重影闪烁效果
6-9秒：音乐重拍，第三款造型，镜头推拉有节奏
9-12秒：音乐重拍，第四款造型，炫影特效
12-15秒：四款造型并排陈列，品牌名渐显

【声音】节奏感强的电子音乐，鼓点清晰
【参考】@image_file_1-4 各款造型，@video_file_1 节奏参考
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "image_files": ["造型1URL", "造型2URL", "造型3URL", "造型4URL"],
  "video_files": ["节奏参考视频URL"],
  "ratio": "9:16",
  "duration": 15
}
```

---

## 示例六：视频延长类

**场景**：延长已有视频

```
将@video_file_1延长10秒（生成长度选择10秒）

延续前视频向日葵滑板的治愈风格：
0-3秒：温暖的午后光线，镜头从街角遮阳篷慢慢下移到墙根小雏菊
3-6秒：主人公的红色板鞋入镜，他蹲在街边花摊前，笑着把向日葵拢进怀里
6-8秒：花瓣蹭过白T恤，他转身踏上滑板，花摊老板笑着喊话
8-10秒：他冲老板挥手，开始滑行，金黄花瓣落在滑板板面

【要求】与前视频保持色调一致，动作连贯流畅
【参考】@video_file_1 原视频
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "video_files": ["原视频URL"],
  "ratio": "16:9",
  "duration": 10
}
```

---

## 示例七：剧情颠覆类

**场景**：改写已有视频剧情

```
基于@video_file_1进行剧情颠覆：

【保留】原视频的古装场景、桥上站位、运镜方式
【修改】0-3秒：男人眼神从温柔瞬间转为冰冷狠厉
【颠覆】3-5秒：在女主毫无防备时，猛地将她推进水里，动作干脆利落，蓄谋已久的决绝
【新增】5-8秒：女主坠入水中，难以置信的眼神，抬头嘶吼"你从一开始就在骗我！"
【新增】8-12秒：男主站在桥上，阴冷笑容，对水面低声说"这是你欠我家族的"
【新增】12-15秒：水面涟漪，画面渐暗

【要求】保持镜头连贯，只在指定位置修改动作和表情
【参考】@video_file_1 原视频素材
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "video_files": ["原视频URL"],
  "ratio": "16:9",
  "duration": 15
}
```

---

## 示例八：一镜到底类

**场景**：追踪镜头穿越多个场景

```
谍战片风格，15秒，2.35:1电影宽屏，紧张氛围

0-3秒：首帧@image_file_1，镜头正面跟拍红衣女特工向前走，全景，路人不断遮挡
3-5秒：镜头跟随到拐角处（参考@image_file_2），固定镜头，女子离开画面
5-8秒：戴面具女孩（参考@image_file_3）在拐角处躲着恶狠狠盯着，镜头往前摇向红衣女子
8-12秒：红衣女子走进豪宅（参考@image_file_4）消失不见
12-15秒：镜头推近豪宅大门，画面定格

【要求】全程不要切镜头，一镜到底，保持紧张节奏
【参考】@image_file_1 首帧，@image_file_2 拐角，@image_file_3 面具女孩，@image_file_4 豪宅
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "image_files": ["首帧URL", "拐角URL", "面具女孩URL", "豪宅URL"],
  "ratio": "21:9",
  "duration": 15
}
```

---

## 示例九：情绪演绎类

**场景**：情绪爆发

```
戏剧化写实风格，10秒，16:9，压抑到爆发的情绪曲线

0-2秒：中景，女子走到镜子前，看着镜中自己，姿势参考@image_file_2
2-4秒：脸部特写，沉思表情，眼神逐渐变化
4-7秒：突然崩溃大叫，抓镜子的动作，情绪和表情完全参考@video_file_1
7-10秒：慢动作，眼泪滑落，画面渐暗

【声音】从安静到突然的爆发音效，情绪配乐
【参考】@image_file_1 女子形象，@image_file_2 姿势参考，@video_file_1 情绪参考
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "image_files": ["女子形象URL", "姿势参考URL"],
  "video_files": ["情绪参考视频URL"],
  "ratio": "16:9",
  "duration": 10
}
```

---

## 示例十：对口型/配音类

**场景**：动物对话搞笑视频

```
搞笑短视频风格，15秒，9:16竖屏，卡通化色彩

0-3秒：@image_file_1的猴子走向奶茶店柜台，镜头跟随在他身后
3-5秒：@image_file_2的比熊服务员在吧台擦拭工具，猴子点单："幺妹儿，霸王别姬有得没得？"（四川口音）
5-8秒：特写服务员，怪异眼神，回答："没得，美式要不要得嘛"
8-12秒：镜头给猴子，挠头念念有词："没事……？我有事！孙儿叫我来买奶茶..."
12-15秒：猴子无奈表情，画面定格

【声音】四川方言对白，奶茶店环境音效
【参考】@image_file_1 猴子形象，@image_file_2 比熊形象
```

**API 参数：**
```json
{
  "functionMode": "omni_reference",
  "image_files": ["猴子形象URL", "比熊形象URL"],
  "audio_files": ["四川方言对白音频URL"],
  "ratio": "9:16",
  "duration": 15
}
```

---

## 示例十一：首尾帧视频

**场景**：日出到日落的延时转场

```
电影级风光摄影风格，8秒，16:9

镜头从日出的温暖金色光芒缓缓过渡到日落的紫红色天际，
远处山脉轮廓始终保持一致，云层快速流动，
光影变化跟随太阳轨迹，氛围从清晨的宁静转变为傍晚的壮丽
```

**API 参数（first_last_frames 模式）：**
```json
{
  "functionMode": "first_last_frames",
  "filePaths": ["日出首帧URL", "日落尾帧URL"],
  "ratio": "16:9",
  "duration": 8
}
```
